...
void fillBCTest(json_spirit::mObject& _o)
{
	...
	_o["genesisRLP"] = toHex(genesisBlock.bytes(), HexPrefix::Add);
	...
	for (auto const& bl: _o["blocks"].get_array())
	{
		...
		blObj["rlp"] = toHex(alterBlock.bytes(), HexPrefix::Add);
...
mObject writeBlockHeaderToJson(BlockHeader const& _bi)
{
	...
	o["extraData"] = (_bi.extraData().size()) ? toHex(_bi.extraData(), HexPrefix::Add) : "";
